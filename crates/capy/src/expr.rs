...
pub(crate) mod block;
pub(crate) mod lambda_call;

use crate::{utils, val::Val, env::Env, lambda::Lambda};
use binding_usage::BindingUsage;
use block::Block;
use lambda_call::LambdaCall;
...
impl Expr {
    pub fn new(s: &str) -> Result<(&str, Self), String> {
        Self::new_lambda(s)
            .or_else(|_| Self::new_non_lambda(s))
    }
    
    fn new_lambda(s: &str) -> Result<(&str, Self), String> {
        LambdaCall::new(s).map(|(s, lambda_call)| (s, Self::LambdaCall(lambda_call)))
            .or_else(|_| Lambda::new(s).map(|(s, lambda)| (s, Self::Lambda(lambda))))
    }

    fn new_non_lambda(s: &str) -> Result<(&str, Self), String>{
    ...
    pub(crate) fn eval(&self, env: &Env) -> Result<Val, String> {
        match self {
            ...
            Self::LambdaCall(call) => call.eval(env),
            Self::BindingUsage(binding_usage) => binding_usage.eval(env),
            Self::Block(block) => block.eval(env),
            Self::Lambda(lambda) => Ok(Val::Lambda(lambda.to_owned()))
        }
    }
}
...
mod tests {
    ...
    fn parse_lambda_call() {
        assert_eq!(
            ...
            Ok((
                "",
                Expr::LambdaCall(LambdaCall {
                    callee: "add".to_string(),
                    args: vec![Expr::Number(Number(1)), Expr::Number(Number(2))]
                })
            ))
        )
    }
